...
array{7} = [0, -0.33, -0.66, -1, -1.33, -1.66, -2, -2.25, -2.534];                 %dangling ends energy - arrayDGDangle
array{8} = [50, 55];                %temperature - arrayTemperatures
array{9} = [20, 7, 4];                 %minimum random match length - arrayMinLength
array{10} = [10000];                   %update step for forced loop rate update - arrayUpdateStep
array{11} = [3, 11, 01]               %structure nr
...
while running
    ...
    results{1, current_parameter_combination + 1} = current_parameter_combination;
    for level = 1:nr_parameters
        results{level + 1, current_parameter_combination + 1} = array{ level }( current_positions( level ) );
    end
